...
export default function Step2Products({ formData, updateForm, onNext, onBack }) {
  ...
  return (
    <div className="bg-white rounded-xl shadow-sm border p-6">
      <h2 className="text-xl font-bold text-gray-800 mb-2">{t('products.title')}</h2>
      <p className="text-sm text-gray-500 mb-4">
        {t('products.order')}: <strong>{formData.deliveryNote?.order_number || formData.deliveryNote?.invoice_number}</strong>
      </p>
      <div className="space-y-2 mb-6">
        {formData.items.map((item) => {
          ...
          return (
            <div key={item.id}
              className={`border rounded-lg p-3 cursor-pointer transition-colors ${isSelected ? 'border-blue-500 bg-blue-50' : 'border-gray-200 hover:border-gray-300'}`}
              ...
              {isSelected && (
                <div className="mt-3 ml-8 flex items-center gap-3" onClick={(e) => e.stopPropagation()}>
                  <label className="text-xs text-gray-500">{t('products.qtyLabel')}</label>
